...
CONFIGS = [
    EvalConfig(benchmark="eval_hard", model_name=HAIKU, max_tool_calls=cap, n_samples=3,
               concurrency=8, label=f"haiku - eval_hard - maxtools={cap} - n3")
    for cap in (6, 12, 18, 24)
] + [
    EvalConfig(benchmark="eval_hard", model_name=HAIKU, max_tool_calls=12, n_samples=3, concurrency=8,
