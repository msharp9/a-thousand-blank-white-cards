...
from agent.nodes import _clear_retriever_cache, retrieve
...
def test_advanced_mode_uses_advanced_retriever() -> None:
    ...
    with patch("agent.nodes.advanced_retriever", return_value=fake_adv):
...
def test_default_mode_uses_dense() -> None:
    ...
    with patch("agent.nodes._retriever", fake_dense):
        out = retrieve(_state())
