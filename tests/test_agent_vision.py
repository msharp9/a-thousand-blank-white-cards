...
def test_setup_create_card_passes_art_to_background_draft() -> None:
    room = Room("ABCDEF")
    room.add_player("p1", "Alice")
    ...
    async def scenario() -> None:
        await room.handle_action("p1", CreateCardMsg(title="Doodle", description="gain 1 point", art=ART))
    ...
    spy.assert_called_once()
    assert spy.call_args.kwargs["card_art"] == ART
    assert spy.call_args.kwargs["draft_mode"] is True
