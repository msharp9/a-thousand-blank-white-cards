...
from logging_config import _CENTRAL_HANDLER_NAME, configure_logging, log_game_state


def _central_handlers() -> list[logging.Handler]:
    root = logging.getLogger()
    return [h for h in root.handlers if getattr(h, "name", None) == _CENTRAL_HANDLER_NAME]


def test_configure_logging_runs_and_sets_level() -> None:
    configure_logging("DEBUG")
    # The level is set on the ROOT logger so every top-level package
    # (board.*, engine.*, agent.*, …) inherits it.
    assert logging.getLogger().level == logging.DEBUG
    assert len(_central_handlers()) == 1
...
def test_log_record_is_formatted(caplog) -> None:
    ...
    assert "engine.test" in formatted
