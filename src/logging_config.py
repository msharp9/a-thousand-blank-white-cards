...
FastAPI lifespan in :mod:`app`) to install a clean, readable formatter
(timestamp, level, logger name, message) on the root logger. Configuring the
root logger means every module logger (and third-party loggers) inherits the
same format without each module doing its own setup.
...
handlers — it only owns the root handler and the root logger level.
"""
...
def configure_logging(level: str | None = None) -> None:
    ...
    handler to the root logger and sets the root logger level. Safe to call
    more than once — the previously-installed handler is swapped out rather
    than duplicated.
    """
    if level is None:
    ...
    # Ensure the root logger honours the configured level explicitly, so every
    # top-level package logger is unaffected if some other library lowers the
    # root level later.
