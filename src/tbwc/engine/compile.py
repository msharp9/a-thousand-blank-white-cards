"""tbwc.engine.compile — compile a card's authoring ops into an EffectProgram.

The card-authoring layer speaks a small, human-friendly op vocabulary (see
``tbwc.models.card.CardOp``): a list of ``{"op": str, "args": dict}`` entries.
This module lowers that vocabulary into the runtime discriminated union of
``tbwc.models.effects`` so the deterministic engine can execute a play without
...
Design notes:
  * Target translation is delegated ENTIRELY to
    ``tbwc.models.effects.map_authoring_target`` — this module never reinvents
