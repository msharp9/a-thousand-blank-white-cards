"""tbwc.rooms.manager — registry of active game rooms.
...
from tbwc.rooms.store import InMemoryRoomStore, RoomStore
...
class RoomManager:
    ...
    process. The default backend (:class:`~tbwc.rooms.store.InMemoryRoomStore`)
...
def check_single_worker(worker_count: int | None = None) -> None:
    ...
    if count is not None and count > 1:
        logger.warning(
            ...
            "shared RoomStore (see tbwc.rooms.store) before scaling out.",
            count,
        )
