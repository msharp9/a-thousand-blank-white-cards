...
testset. The A/B driver scripts already exist (`tbwc.evals.retriever_ab`,
`tbwc.evals.improvement_ab`), but running them end-to-end requires a live
...
`advanced_retriever()` in [`src/tbwc/rag/retrievers.py`](../rag/retrievers.py)).
...
- Scores every run with the full scorer set `ALL_SCORERS`
  ([`src/tbwc/evals/scorers.py`](./scorers.py)):
...
([`src/tbwc/evals/judge.py`](./judge.py)) that returns a structured `Verdict`; only
...
`emit_ops` ([`src/tbwc/agent/nodes.py`](../agent/nodes.py)) produces the
...
The driver is [`src/tbwc/evals/improvement_ab.py`](./improvement_ab.py). It runs the
...
```bash
uv run python -m tbwc.evals.improvement_ab
```
...
```bash
uv run python -m tbwc.evals.retriever_ab   > retriever_ab_results.md
uv run python -m tbwc.evals.improvement_ab > improvement_ab_results.md
```
